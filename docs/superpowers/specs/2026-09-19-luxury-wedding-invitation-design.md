...
- **Date**: 2026-09-19
- **Aesthetic Direction**: Editorial High-Fashion / Modern Classic (Kinfolk / Vogue style)
...
- Teks personalisasi di atas amplop: *"Special Invitation for [Nama Tamu]"*.
- Monogram lilin interaktif: Menampilkan inisial kedua mempelai.
...
- Tombol mengambang elegan di pojok layar berbentuk piringan vinyl dengan label monogram.
...
- Menampilkan Surat Ar-Rum ayat 21 dengan teks kaligrafi Arab berkualitas tinggi (vektor/Amiri font) dan terjemahan bahasa Indonesia.
- Menampilkan doa berkah pernikahan: *"Barakallahu laka wa baraka 'alaika wa jama'a bainakuma fii khair"*.
- Menggunakan animasi kemunculan bertingkat (*staggered fade-up reveal*).
...
- Galeri foto format majalah dengan *masonry layout*.
...
- Menampilkan rekening bank resmi pengantin dengan tombol *one-click copy to clipboard* disertai notifikasi tooltip.
- Modal tampilan QRIS resolusi tajam untuk memudahkan donasi digital.

#### G. RSVP & Live Guestbook Wall
- Form konfirmasi kehadiran: Nama Tamu, Status (Hadir / Berhalangan), Jumlah Tamu (1–5 orang), Doa Restu.
- Efek perayaan konfeti instan (`canvas-confetti`) setelah submit sukses.
- Daftar ucapan tamu tampil secara *real-time* menggunakan Supabase Realtime WebSocket tanpa perlu memuat ulang halaman.
...
| **Dynamic OpenGraph** | `@vercel/og` (Satori) | Generator kartu gambar preview WA dinamis |
